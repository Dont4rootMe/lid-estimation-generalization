...
данных. Протокол v13 включает11 активных методов и39 представлений (429 ячеек).
PFGM++ временно исключён из запуска и таблиц; его код и ядро сохранены.
Десять полевых методов используют U-Net width8 на изображениях и point residual
width512 на коэффициентах, без пространственных Fourier-признаков. Native
входы, targets и losses описаны в [постановках методов](docs/native_methods.md).
NF остаётся одной обратимой моделью с scalar log(σ)-conditioning без частот;
её ёмкость согласована с полями, image-NF использует два conditioner width5.
Общие правила обучения и выбора29 модельных λ и Kneedle по log(λ)
