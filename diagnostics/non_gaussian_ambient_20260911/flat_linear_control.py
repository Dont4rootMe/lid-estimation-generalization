"""One bounded full-rank learned-linear shortcut diagnostic.

Compare with the already retained noise_v1 control on the identical synthetic
draw sequence. Every entry of a30x30 matrix is trained by the native objective;
no covariance, basis, target rank or analytic normal posterior enters the model.
This is not an adopted benchmark architecture or an Exp/Spiral training.
"""
import hashlib
import json
...
OUT=BASE/'flat_linear_control'


class LearnedLinearFixture(nn.Module):
    def __init__(self,core):
        super().__init__();self.core=core
        self.linear=nn.Linear(30,30,bias=False)
        nn.init.zeros_(self.linear.weight)
        self.coefficient='noise_v1_plus_full_learned_linear'
    ...
    def forward(self,y,scale):
        ...
        return (y+self.linear(y))*inverse.square()+scale[:,None]*inverse*residual
...
def main():
    ...
    model=LearnedLinearFixture(build_model('t_flowmatching',cfg,30).core.cuda()).cuda()
    ...
    manifest=dict(status='in_progress',steps=4000,data_seed=28517,model_seed=3981,batch_size=256,
        ...
        additional_trainable_parameters=900,known_span_in_model=False,
        initialization='zero unrestricted30x30 shortcut, identical baseline core',
